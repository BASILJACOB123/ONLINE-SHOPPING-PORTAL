...
if($_GET['submit'])
{
  ...
  $s="select * from customer_table where Email_id='$email_id'";
  $result=mysqli_query($conn,$s);
  $num=mysqli_num_rows($result);
  if($num>=1){
      echo "<p style='color:white;margin-left:600px;font-size:30px;'> Email already taken Please sign in </p> ";
  
  }
  else
  {

        $query ="INSERT INTO customer_table(customer_id,Email_id,Address,Name,Payment_id,Password) values ('$customer_id','$email_id','$addr','$name','$Pay_id','$Pass')";
    $data=mysqli_query($conn,$query); 
    if($data)
    {
      echo "<p style='color:white;margin-left:600px;font-size:30px;'>Data inserted into the database</p>";
  ?>
      <META HTTP-EQUIV="refresh" CONTENT="0; URL='http://localhost:8080/basil_jacob/DBMS_project.php?email_id=<?php echo $email_id ?>&customer_id=<?php echo $customer_id ?>&addr=<?php echo $addr ?>&name=<?php echo $name ?>';">
      <?php
